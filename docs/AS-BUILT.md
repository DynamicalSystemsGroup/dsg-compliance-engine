...
- **Validation (human).** Only the Affirming Official's Gate-2 attestation
  (**`ce:attests` + `ce:hasOutcome earl:passed`**) makes a control MET. For a
  **Track B (policy/human)** control, the **attested-reference oracle is now wired
  into the run path** and gates MET with teeth: the control is MET **only if** the
  oracle passes (reference registered, resolves to a real file, within its
  freshness window, signed by the required role). A stale, dead-link, missing, or
  wrong-signer reference yields `needsAction`/`failed`, the control is **not MET**,
  and the SPRS score drops (verified: staling the SSP reference for `CA.L2-3.12.4`
  moved 110/Final → 109/Conditional). Only where no oracle applies at all does the
  control fall back to human-only. _Evidence addresses, only attestation attests._
...
| **R4**  | MET **only when evidence passes AND a human attests**; policy controls route through the attested-reference oracle → `needsAction`/`failed` | `oracles/`, `traceability/attestation.py`                                                                                                                        | machine oracle `earl:{passed,failed}`, attested-reference oracle `earl:{passed,needsAction,failed}`; every required control resolves to a concrete outcome; MET requires `ce:attests`+`earl:passed`                                                                                         |
...
| **R13** | **Contradiction detector (with score teeth)** + proven-vs-attested split, surfaced in the SSP             | `ContradictionShape` (ontology), `traceability/attestation.py` (`ce:oracleOutcome`/`cmmc:overrideJustification`), `traceability/audit.py`, `traceability/sprs.py`, `documents/ssp.py` (colophon) | MET over `failed`/absent oracle w/o override = violation; the contradicted control does **not** count as MET (weight deducted from SPRS) and `valid_submission=False`. Verified: the `contradiction` scenario now scores 105/Conditional/invalid, not 110/Final. "N MET-by-machine / M MET-by-human-only" flows into the SSP colophon via `SprsSummary` |
...
| Attested-reference oracle + document evidence (`oracles/attested_reference.py`, `pipeline/evidence/doc_evidence.py`) | **Wired, with teeth.** The attested-reference oracle now runs in the run path (`pipeline/runner.py` `_run_attested_reference_pass`) and gates MET for Track B controls: it checks the reference is registered, resolves to a real file, is within its freshness window, and is signed by the required role, emitting `passed`/`needsAction`/`failed` with a machine-readable reason. For each Track B control, `doc_evidence.py` resolves the control's `ce:Reference` URI to the real document on disk, SHA-256 hashes it, captures the git commit that last changed it (author + date), and signs an upload receipt with the local Ed25519 signer, binding all of this into the evidence graph as a `ce:DocumentEvidence` node. The `references.ttl` reference nodes are loaded into the run dataset via `order_compiler/compiler.py` `load_pipeline_dataset`. This produces the BOM `attested-evidenced` backing (see next row). The upload receipt uses the local Ed25519 dev signer (same key path as the demo, not production cosign/KMS); the Flexo-versioned anchor for the node materializes only under `--store-backend flexo`. Runs remain NON-EVIDENTIARY (evidence is still fixture/mock). |
| BOM `evidence_backing` (`traceability/bom.py`)      | Four values: `machine` (passing config-check oracle over resolvable evidence), `override` (MET over a failed check, with justification + appended override evidence), **`attested-evidenced`** (a human attestation backed by machine-recorded document evidence: resolved + hashed + git commit + signed upload receipt), and `human-only` (human judgment with no machine anchor). `attested-evidenced` rows also carry `reference_id`, `git_commit`, and `git_committed_at`, and their attestation rationale is the real Affirming Official sign-off text and signer name from `data/attestations/tier1.jsonl` (via `attestation_store.load_all`), not boilerplate. Over the full 110-control catalog (`ce demo --full`) the distribution is 65 machine / 43 attested-evidenced / 2 CSP-inherited (`ce:inherited`) — no control is silently `human-only`: every machine control has a criterion and a mock fixture feeding it, and a control with no result would be `needsAction`, not MET. The score stays 110/Final because all evidence, references, and AO records are complete and fresh. |
