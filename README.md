...
- **Track B (attested reference).** 16 modules, 43 controls: the policy-and-records controls (training, incident response, risk assessment, personnel security, configuration management, maintenance, media protection, audit procedure, security engineering, remote-access authorization, physical access, collaborative computing, separation of duties, continuous monitoring, and login banner). These controls are now backed by machine-recorded document evidence rather than a bare human sign-off: over the full 110-control catalog (`ce demo --full`), the backing distribution is 65 machine, 43 attested-evidenced, and 2 CSP-inherited. No control is silently attested MET: every machine control has a criterion and a mock evidence fixture feeding it, CSP-inherited controls carry an explicit `inherited` outcome, and anything without a result would be `needsAction` (not MET). The score is 110/Final because all evidence, references, and Affirming Official records are complete and fresh; staling a single reference (for example the SSP reference for `CA.L2-3.12.4`) drops the score below Final.
...
Each control row in the BOM carries an `evidence_backing` value recording how the sign-off is supported. There are four: `machine` (a passing config-check oracle over resolvable evidence), `override` (met over a failed check, requiring a written justification plus appended override evidence), `attested-evidenced` (a human attestation backed by machine-recorded document evidence), and `human-only` (a human judgment with no machine anchor). The `attested-evidenced` backing is what a Track B control earns when its attested-reference chain holds: the engine resolves the control's `ce:Reference` URI to the real document on disk, SHA-256 hashes it, captures the git commit that last changed it (author and date), signs an upload receipt with the local Ed25519 signer, and binds all of this into the evidence graph as a `ce:DocumentEvidence` node. The attested-reference oracle then gates the control: it is marked MET only if the reference is registered, resolves to a real file, is within its freshness window, and is signed by the required role. A stale, dead-link, missing, or wrong-signer reference yields `needsAction` or `failed`, the control is not MET, and the SPRS score drops accordingly. The `attested-evidenced` rows also carry the `reference_id`, `git_commit`, and `git_committed_at` so an assessor can trace the sign-off to an exact document version. The attestation rationale on these rows is the real Affirming Official sign-off text and signer name drawn from `data/attestations/tier1.jsonl`, not boilerplate.

The document's substance is still the human's judgment: the engine checks the bureaucratic facts (the reference resolves, is fresh, and is role-signed), not whether the underlying policy is actually good. The evidence remains fixture/mock, so runs stay non-evidentiary; the upload receipt uses the local Ed25519 dev signer (the same key path as the rest of the demo, not production cosign/KMS); and the Flexo-versioned anchor for the `ce:DocumentEvidence` node only materializes when run with `--store-backend flexo`, which carries the node into the append-only store with the rest of the evidence graph.
